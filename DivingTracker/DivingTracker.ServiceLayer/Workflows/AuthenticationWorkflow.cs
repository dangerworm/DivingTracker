...
namespace DivingTracker.ServiceLayer.Workflows
{
    public class AuthenticationWorkflow
    {
        ...
        public DataResult<User> Confirm(Guid emailConfirmationToken)
        {
            ...
            if (systemLogin == null)
                return new DataResult<User>(DataResultType.NoRecordsFound,
                    "Could not find a login with that email token");
            ...
            return new DataResult<User>(user,
                new DataResult(DataResultType.Success, "Successfully confirmed user's email address"));
        }
        ...
        public DataResult<User> Login(string emailAddress, string password)
        {
            ...
            if (!systemLogin.IsEmailConfirmed)
                return new DataResult<User>(DataResultType.ConfirmationRequired,
                    "This login has not had the email address confirmed");
            ...
            return AreByteArraysEqual(saltedHash, dbSaltedHash)
                ...
                : new DataResult<User>(DataResultType.Unauthorised,
                    "The username/password combination is not recognised.", "Authentication failed.");
        }
