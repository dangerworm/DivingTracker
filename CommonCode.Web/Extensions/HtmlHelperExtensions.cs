    ...
    public static class HtmlHelperExtensions
    {
            ...
            int labelWidth = 12, object htmlAttributes = null)
        {
            ...
            else
            {
                ...
                output.Append(@"<div class=""row form-group"">");
                output.Append(html.LabelFor(expression, metadata.Description,
                    new {@class = $"col-sm-{labelWidth} col-form-label"}));
            ...
            switch (controlType)
            {
                ...
                case HtmlHelperExtensionsControlTypes.TextArea:
                    output.Append(html.TextAreaFor(expression,
                        htmlAttributes ?? new {@class = "form-control", placeholder = metadata.Watermark, rows = 5}));
                    break;

                case HtmlHelperExtensionsControlTypes.TextBox:
                    output.Append(html.TextBoxFor(expression,
                        htmlAttributes ?? new {@class = "form-control", placeholder = metadata.Watermark}));
                    break;
            }
        ...
        {
            return FormGroupControlHorizontalFor(html, expression, HtmlHelperExtensionsControlTypes.Password,
                labelWidth, htmlAttributes);
        }

        public static MvcHtmlString FormGroupTextAreaHorizontalFor<TModel, TResult>(this HtmlHelper<TModel> html,
            Expression<Func<TModel, TResult>> expression, int labelWidth = 12, object htmlAttributes = null)
        {
            return FormGroupControlHorizontalFor(html, expression, HtmlHelperExtensionsControlTypes.TextArea,
                labelWidth, htmlAttributes);
        }
        ...
        public static MvcHtmlString FormGroupTextBoxHorizontalFor<TModel, TResult>(this HtmlHelper<TModel> html,
            Expression<Func<TModel, TResult>> expression, int labelWidth = 12, object htmlAttributes = null)
        {
            return FormGroupControlHorizontalFor(html, expression, HtmlHelperExtensionsControlTypes.TextBox, labelWidth,
                htmlAttributes);
        }
